                ...
                <div class="row mb-3">
                    ...
                    <div class="col-lg-4 mb-2">
                        ...
                        <select class="form-control select2-enabled" id="bom_id" name="bom_id" required style="width:100%;">
                            ...
                            ?>
                                <option value="<?php echo htmlspecialchars($bom['id']); ?>" <?php echo ($jci_data['bom_id'] == $bom['id']) ? 'selected' : ''; ?>>
...
$('#sell_order_id').on('change', function(e) {
    ...
    <?php if ($edit_mode): ?>
        ...
        console.log('JCI data from PHP:', <?php echo json_encode($jci_data); ?>);
        var initialSellOrderId = '<?php echo htmlspecialchars($sell_order_selected); ?>';
        console.log('Initial sell order ID:', initialSellOrderId);
        console.log('Item data from PHP:', <?php echo json_encode($item_data); ?>);
        
        // Always load existing data directly in edit mode
        console.log('Loading existing data directly in edit mode');
        ...
        console.log('JCI data loaded:', jciData);
        ...
        if (itemData && itemData.length > 0) {
            ...
            if (jciData && jciData.po_id) {
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function(productResponse) {
                        console.log('Products loaded for edit mode:', productResponse);
                        if (productResponse.success && productResponse.products.length > 0) {
                            poProducts = productResponse.products;
                            ...
                            if (initialSellOrderId) {
                                ...
                                console.log('Set sell order dropdown value:', initialSellOrderId);
                            }
                            ...
                            if (jciData.bom_id) {
                                setTimeout(function() {
                                    ...
                                    console.log('Set BOM dropdown value:', jciData.bom_id);
                                }, 200);
                            }
                            ...
                            itemData.forEach(function(item) {
                                console.log('Adding row with item:', item);
                                addRow(item);
                            });
            ...
            console.log('No item data, adding empty row');
            addRow();
        }
    ...
    <?php else: ?>
        ...
        console.log('Add mode - no automatic rows');
    <?php endif; ?>
    ...
    $('#sell_order_id').off('select2:select').on('select2:select', function(e) {
        var sellOrderId = $(this).val();
        console.log('Sell order selected:', sellOrderId);
