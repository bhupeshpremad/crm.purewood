...
if (!in_array($bill_image['type'], $allowed_types)) {
    echo json_encode(['success' => false, 'error' => 'Invalid bill image type']);
    exit;
}

if (!in_array($builty_image['type'], $allowed_types)) {
    echo json_encode(['success' => false, 'error' => 'Invalid builty image type']);
    exit;
}
...
$bill_image_name = uniqid('invoice_') . '_' . basename($bill_image['name']);
$builty_image_name = uniqid('builty_') . '_' . basename($builty_image['name']);
