...
try {
    ...
    $bom_glow_data = [];
    $stmt_glow = $conn->prepare("SELECT 'Glow' as product_type, glowtype as product_name, quantity, price FROM bom_glow WHERE bom_main_id = ?");
    $stmt_glow->execute([$bom_id]);
    $bom_glow_data = $stmt_glow->fetchAll(PDO::FETCH_ASSOC);

    $bom_hardware_data = [];
    $stmt_hardware = $conn->prepare("SELECT 'Hardware' as product_type, itemname as product_name, quantity, price FROM bom_hardware WHERE bom_main_id = ?");
    ...
    $bom_wood_data = [];
    $stmt_wood = $conn->prepare("SELECT 'Wood' as product_type, woodtype as product_name, quantity, price FROM bom_wood WHERE bom_main_id = ?");
