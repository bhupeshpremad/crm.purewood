...
<script>
    $(document).ready(function() {
        ...
        // Trigger change on PTM amount to recalculate totals
        $('#payment_details_table').on('input', '.ptm_amount', function() {
            calculatePaymentAmounts();
        });
        ...
        function calculatePaymentAmounts() {
            ...
            $('#payment_details_table tbody .payment-row').each(function() {
                // Only consider checked and enabled rows for total calculation
                const $selectPaymentCheckbox = $(this).find('.select_payment');
                if ($selectPaymentCheckbox.is(':checked') && !$selectPaymentCheckbox.is(':disabled')) {
                    let ptmAmount = parseFloat($(this).find('.ptm_amount').val()) || 0;
                    totalPaymentAmount += ptmAmount;
                } else if ($selectPaymentCheckbox.is(':checked') && $selectPaymentCheckbox.is(':disabled')) {
                    // Include read-only (already paid) amounts in total
                    let ptmAmount = parseFloat($(this).find('.ptm_amount').val()) || 0;
                    totalPaymentAmount += ptmAmount;
                }
            });
        ...
        $('#vendorPayment_form').on('submit', function(e) {
            ...
            // Validate that all selected payments have required fields
            let hasIncompletePayments = false;
            $('#payment_details_table tbody .select_payment:checked').each(function() {
        ...
        function submitFormData() {
            ...
            $('#payment_details_table tbody .payment-row').each(function(index) {
                let row = $(this);
                // Only include checked rows with complete data
                if (!row.find('.select_payment').is(':checked')) {
                ...
                if (!chequeType || !chequeNumber || !paymentDate) {
                    return true; // skip incomplete rows
                }
                paymentsData.push({
