                ...
                if (response.success) {
                    ...
                    if (response.payments.length > 0) {
                        response.payments.forEach(function(payment, index) {
                            var paymentStatus = payment.payment_count > 0 ? 
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.payment_details && response.payment_details.length > 0) {
                    ...
                    $('#paymentDetailsTable tbody').append('<tr><td colspan="9" class="text-center">No payment details found.</td></tr>');
                }
                $('#totalPaymentAmount').text(totalAmount.toFixed(2));
                $('#paymentDetailsModal').modal('show');
            },
            error: function(xhr, status, error) {
                $('#paymentDetailsTable tbody').append('<tr><td colspan="9" class="text-center">Error loading payment details.</td></tr>');
                $('#paymentDetailsModal').modal('show');
            }
        });
    });
