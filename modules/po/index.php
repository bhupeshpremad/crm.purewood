...
$stmt = $conn->prepare("SELECT id, po_number, client_name, prepared_by, created_at, updated_at, status, is_locked FROM po_main ORDER BY created_at DESC");
$stmt->execute();
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($po['is_locked'] == 0): ?>
                                    ...
                                    <?php endif; ?>
                                    <?php if ($po['status'] == 'Approved' && $po['is_locked'] == 0): ?>
                                        <button class="btn btn-sm btn-danger lock-po-btn" data-po-id="<?php echo $po['id']; ?>">Lock</button>
                                    <?php endif; ?>
                                <?php else: ?>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="poItemDetailsModalLabel">PO Item Details</h5>
        ...
        btn.addEventListener('click', function() {
            ...
            .then(data => {
                ...
                if (data.success && data.items.length > 0) {
                    ...
                    tbody.innerHTML = '<tr><td colspan="8" class="text-center">No items found for this PO.</td></tr>';
                } else {
                    tbody.innerHTML = `<tr><td colspan="8" class="text-center text-danger">Error: ${data.message || 'Failed to load items.'}</td></tr>`;
                }
            ...
            .catch(error => {
                const tbody = document.querySelector('#poItemDetailsTable tbody');
                tbody.innerHTML = `<tr><td colspan="8" class="text-center text-danger">Network error or failed to parse response: ${error.message}</td></tr>`;
                var modal = new bootstrap.Modal(document.getElementById('poItemDetailsModal'));
                modal.show();
